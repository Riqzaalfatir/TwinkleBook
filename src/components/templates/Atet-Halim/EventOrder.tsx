...
type EventOrderProps = {
  data?: any; // ✅ ini assembledData penuh, bukan array langsung
};
...
const EventOrder = ({ data }: EventOrderProps) => {
  ...
  // ✅ Filter session yang beneran milik event ini (buang entry eventId nol/sampah)
  //    lalu urutin berdasarkan tanggal, bukan nama string
  const validSessions = rawSessions
    .filter((s: any) => s.eventId === currentEventId)
    .sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());

  // ✅ Ambil 2 sesi pertama secara urutan (ceremony = paling awal, reception = berikutnya)
