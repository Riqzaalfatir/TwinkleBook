...
const EventOrder = ({ data }: EventOrderProps) => {
  ...
  const rawSessions = data?.dataSession ?? [];

  const validSessions = rawSessions
    .filter((s: any) => s.eventId === currentEventId)
    .sort(
      (a: any, b: any) =>
        new Date(a.date).getTime() - new Date(b.date).getTime(),
    );

  const ceremonySession = validSessions[0] ?? {
    name: "Thanksgiving Mass",
    date: "2026-09-05T10:00:16",
    addressName: "PULLMAN GRAND BALLROOM",
    address:
      "Pullman Bandung Grand Central, Jalan Diponegoro, Citarum, Bandung City, West Java, Indonesia",
    latLong: "-6.9002323, 107.6174782",
  };

  const receptionSession = validSessions[1] ?? {
  ...
  // Format waktu
  const ceremonyTime = new Date(ceremonySession.date).toLocaleTimeString(
    "en-US",
    {
      hour: "2-digit",
      minute: "2-digit",
  ...
  const receptionTime = new Date(receptionSession.date).toLocaleTimeString(
    "en-US",
    {
      hour: "2-digit",
      minute: "2-digit",
  ...
  const mapsLink =
    ceremonySession.mapsUrl ?? "https://maps.app.goo.gl/2iNGFtQ4yKTuH9Gi7";
        ...
        <div className="relative w-full bg-[url('/images/Atet-Halim/EventOrder/LayerKertas.webp')] bg-no-repeat [background-size:100%_100%] lg:bg-[url('/images/Atet-Halim/EventOrder/LayerKertasD.svg')] lg:[background-size:100%_100%]">
          <div className="relative flex flex-col items-center text-center">
            ...
            <motion.p
              ...
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 1.5, ease: "easeOut" }}
              className="font-athelas text-[3.59vw] lg:text-[1.45vw] lg:pt-[5px] text-[#402824] max-w-[80vw]  lg:max-w-[35vw] break-words"
            >
              {fullAddress}
            </motion.p>
            ...
            <motion.h4
              ...
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 1.5, ease: "easeOut" }}
              className="font-cylburn text-[10.26vw] lg:text-[3.43vw] text-[#402824] pt-[13.33vw] lg:pt-[4.7vw] [--stroke-w:0.5px] lg:[--stroke-w:0.8px] leading-none max-w-[65vw] lg:max-w-[30vw] break-words"
              style={{ WebkitTextStroke: "var(--stroke-w) #402824" }}
            >
              {ceremonySession.name}
            </motion.h4>
            <motion.p
              variants={fadeUp}
              initial="hidden"
            ...
            <motion.h4
              ...
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 1.5, ease: "easeOut" }}
              className="font-cylburn text-[10.26vw] lg:text-[3.43vw] text-[#402824] pt-[9.74vw] lg:pt-[2.58vw]  [--stroke-w:0.5px] lg:[--stroke-w:0.8px] leading-none max-w-[65vw] lg:max-w-[30vw] break-words"
              style={{ WebkitTextStroke: "var(--stroke-w) #402824" }}
            >
              {receptionSession.name}
            </motion.h4>
            <motion.p
              variants={fadeUp}
              initial="hidden"
